  ...
  <div>
    ...
    <div class="relative">
      <input
        ...
        :disabled="disabled"
        class="block w-full rounded-lg border border-zinc-300 bg-white px-3 py-2 text-sm text-zinc-900 shadow-xs placeholder:text-zinc-400 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 focus:outline-none disabled:bg-zinc-100"
        :class="[mono && 'font-mono', isPassword && 'pr-10']"
      />
      <button
        v-if="isPassword"
        type="button"
        class="absolute inset-y-0 right-0 flex w-10 items-center justify-center text-zinc-400 hover:text-zinc-700 focus:outline-none focus-visible:text-indigo-600"
