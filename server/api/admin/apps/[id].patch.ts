...
export default defineEventHandler(async (event) => {
  ...
  if (input.credentialId) {
    ...
    if (!credential || credential.userId !== existing.userId) {
  ...
  if (input.slug) {
    ...
    if (taken) throw createError({ statusCode: 409, statusMessage: 'Conflict', message: `The slug "${input.slug}" is already in use, choose another` })
  }
